...
class PostForm(FlaskForm):
    ...
    image = FileField('이미지 업로드',validators=[FileRequired(), 
        FileAllowed(['jpg', 'png'], 'JPG와 PNG 이미지만 업로드 가능합니다.')])

# 계정생성을 위한 폼

class UserCreateForm(FlaskForm):
    account = StringField('아이디', validators=[
        DataRequired(), Length(min=5, max=20)])  # 길이 5~20
    name = StringField(
        '이름', validators=[DataRequired(), Length(min=2, max=35)])
    nickname = StringField(
        '닉네임', validators=[DataRequired(), Length(min=2, max=20)])
...
class PasswordChangeForm(FlaskForm):
    ...
    new_password1 = PasswordField(
        '새 비밀번호', validators=[
            DataRequired(),
            EqualTo('new_password2', '새 비밀번호가 일치하지 않습니다')])
